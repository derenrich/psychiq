...
WITH parsed_targets AS 
 (
  ...
  parsed_map AS (
   ...
   SELECT title,qid,page_id, enwiki FROM `wikidata-319717.bq.enwiki_texts` join parsed_map ON title=UnescapeTitle(enwiki) 
 ), cats AS (
 ...
 ) SELECT CONCAT(cat_text,"\n",title) as text, enwiki, qid from title_texts join cats_text on cats_text.page_id = title_texts.page_id where qid not in (select distinct source from parsed_targets)
